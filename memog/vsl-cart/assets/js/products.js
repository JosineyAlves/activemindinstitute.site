...
product.forEach(function (e) {
	...
	let shipping = typeof e.dataset.shipping !== 'undefined'
		? `<div class="shipping">+&nbsp;<span>${e.dataset.shipping}</span> ${document.body.classList.contains('digistore24') ? 'US SHIPPING' : 'SHIPPING'}</div>`
		: "";
	let cards = "";
	...
	*/
	...
	} else if (['upsell1', 'upsell2', 'upsell3', 'downsell1', 'downsell2'].includes(document.body.id)) {
	...
	// Cards images removed - not present in img folder
	if (offer == 'promo') cards = "";
	else cards = "";
    ...
    <div class="item ${offer}">
        <div class="wrapper">
            ...
            <div class="item-img">
                ...
                <img src="${image}" alt="${bottles}">
            </div>
            <div class="item-info">
                <div class="price">
                    <b><sup>$</sup>${price}</b>
                    ${bottles == 1 ? '' : '<span>PER<br>BOTTLE</span>'}
                </div>
                <div class="savings">
            ...
            <div class="item-buy">
                ...
                ${cards ? `<img src="${cards}" alt="Cards" class="card-flags">` : ''}
            </div>
...
window.addEventListener('DOMContentLoaded', () => {
	...
	if (bodyClassList.contains('clickbank') && !(pageParam || document.body.id === 'vsl' || document.body.id === 'dtc' || document.body.id === 'ecom')) {
